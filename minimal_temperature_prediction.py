...
# Load transformer
checkpoint = list(Path("models/transformer/xformer_rep_00/lightning_logs/version_0/checkpoints").glob("best-*.ckpt"))[0]
model_config = ModelConfig(model_type="transformer", seq_length=1164, embedding_dim=256, num_layers=3, nhead=4, dim_feedforward=1024)
train_config = TrainConfig(phenotypes=phenotype_names)
transformer = Transformer.load_from_checkpoint(checkpoint, model_config=model_config, train_config=train_config)
transformer.eval()
...
arbitrary_temp = 28.5
results = [interp1d(temp_values, temp_predictions[i].numpy(), kind='cubic', fill_value='extrapolate')(arbitrary_temp) for i in range(1000)] # B=1000, P=1
